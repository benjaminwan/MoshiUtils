...
inline fun <reified T> toJsonByMoshi(src: T): String {
    val jsonAdapter = Moshi.Builder().add(KotlinJsonAdapterFactory()).build().adapter<T>(T::class.java)
...
inline fun <reified T> fromJsonToListByMoshi(json: String): List<T>? {
    ...
    val jsonAdapter = Moshi.Builder().add(KotlinJsonAdapterFactory()).build().adapter<List<T>>(listType)
    try {
        return jsonAdapter.fromJson(json)
    } catch (e: Exception) {
        e.printStackTrace()
...
inline fun <reified T1, reified T2> fromJsonToMapByMoshi(json: String): Map<T1, T2>? {
    ...
    val jsonAdapter = Moshi.Builder().add(KotlinJsonAdapterFactory()).build().adapter<Map<T1, T2>>(mapType)
    try {
        return jsonAdapter.fromJson(json)
    } catch (e: Exception) {
        e.printStackTrace()
